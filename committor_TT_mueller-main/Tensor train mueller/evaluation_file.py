...
# +
import pandas as pd
FD_file = pd.read_excel('FEMdata_T30_uniform_rugged.xlsx')
...
Test_data_dataframe.to_excel('test_data_T30_rugged.xlsx',index=False) #save to file
...
# +
import pandas as pd
FD_file = pd.read_excel('FEMdata_T30_uniform.xlsx')

points_x = pd.DataFrame(FD_file['X'])
points_y = pd.DataFrame(FD_file['Y'])
target = pd.DataFrame(FD_file['TARGET'])

x3 = np.random.uniform(-1,1,np.array(points_x.values).shape[0])
x4 = np.random.uniform(-1,1,np.array(points_x.values).shape[0])
pos = np.hstack((np.array(points_x.values),np.array(points_y.values),x3[:,None],x4[:,None]))
...
# +
Test_data = np.hstack((new_pos,new_target))
Test_data_dataframe= pd.DataFrame(Test_data,columns=['X1','X2','X3','X4','committor']) #convert to a dataframe

Test_data_dataframe.to_excel('test_data_T30_4D.xlsx',index=False) #save to file
